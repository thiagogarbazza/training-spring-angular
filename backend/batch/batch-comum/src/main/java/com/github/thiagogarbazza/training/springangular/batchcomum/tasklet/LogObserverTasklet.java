package com.github.thiagogarbazza.training.springangular.batchcomum.tasklet;
...
@Component
@CommonsLog
class LogObserverTasklet {

  @Around("execution(public * org.springframework.batch.core.step.tasklet.Tasklet+.execute(..))")
  ...
  private void afterPoint(final ProceedingJoinPoint joinPoint, final Object resposta, final LocalDateTime start) {
    if (log.isInfoEnabled()) {
      log.info(format(MSG_LOG_ENDING, MSG_SYMBOL_H1, toStringSignatureBuilder(joinPoint), resposta, between(start, now())));
    }
  }

  private void beforePoint(final ProceedingJoinPoint joinPoint) {
    if (log.isInfoEnabled()) {
      log.info(format(MSG_LOG_START, MSG_SYMBOL_H1, toStringSignatureBuilder(joinPoint)));
    }
  }
  ...
  private Object executePoint(final ProceedingJoinPoint joinPoint, final LocalDateTime start) throws Throwable {
    try {
      ...
      log.error(format(MSG_LOG_ERROR, MSG_SYMBOL_H1, toStringSignatureBuilder(joinPoint), throwable.getMessage(), between(start, now())));
